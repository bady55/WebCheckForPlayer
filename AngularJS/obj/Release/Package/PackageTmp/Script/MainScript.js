...
contr.controller('ApiController', function ($scope, $http) {
    ...
    $scope.GetInfoByLogshets = function (data) {
        ...
        for (var i in res) {
            $scope.InfoList.push({ Time: res[i].Time, Type: '???', Keywords: res[i].Keyword.toString(), Names: res[i].Name.toString(), Quality: res[i].Quality });
        }
    }
    $scope.LoadLogSheets = function () {
        var res = $scope.sendRequest(LOGSHEETURL + '/GetLogsheetByGroupId', 'GET', { groupId: GROUPID });
        for (var i in res) {
